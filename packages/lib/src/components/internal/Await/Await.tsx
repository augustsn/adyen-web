...
function Await(props: AwaitComponentProps) {
    ...
    const onTimeUp = (): void => {
        ...
        props.onError(new BubpCheckoutError('ERROR', 'Payment Expired'));
    };
    ...
    const onComplete = (status: StatusObject): void => {
        ...
        setExpired(true);
        props.onError(new BubpCheckoutError('ERROR', 'successful result, but no payload in response'));
    };
    ...
    const onError = (status: StatusObject): void => {
        ...
        props.onError(new BubpCheckoutError('ERROR', 'error result with no payload in response'));
    };
    ...
    const finalState = (image, message) => (
        <div className="bubp-checkout__await bubp-checkout__await--result">
            <img
                className="bubp-checkout__await__icon bubp-checkout__await__icon--result"
                src={getImage({ imageFolder: 'components/' })(image)}
                alt={i18n.get(message)}
            />
            <div className="bubp-checkout__await__subtitle bubp-checkout__await__subtitle--result">{i18n.get(message)}</div>
        </div>
    );
    ...
    if (loading) {
        return (
            <div className="bubp-checkout__await">
                {props.brandLogo && <img src={props.brandLogo} alt={props.type} className="bubp-checkout__await__brand-logo" />}
    ...
    return (
        <div
            className={classnames(
                'bubp-checkout__await',
                `bubp-checkout__await--${props.type}`,
                props.classNameModifiers.map(m => `bubp-checkout__await--${m}`)
            )}
        >
            {props.brandLogo && <img src={props.brandLogo} alt={props.type} className="bubp-checkout__await__brand-logo" />}
            ...
            {!!(props.showAmount && amount?.value && amount?.currency) && (
                <div className="bubp-checkout__await__amount">{i18n.amount(amount.value, amount.currency)}</div>
            )}

            <div className="bubp-checkout__await__subtitle">{props.messageText}</div>

            <div className="bubp-checkout__await__indicator-holder">
                <div className="bubp-checkout__await__indicator-spinner">
                    <Spinner inline={false} size="medium" />
                </div>
                <div className="bubp-checkout__await__indicator-text">{props.awaitText}</div>
            </div>

            {props.showCountdownTimer && (
                <div className="bubp-checkout__await__countdown-holder">
                    <div className="bubp-checkout__await__progress">
                        <span className="bubp-checkout__await__percentage" style={{ width: `${percentage}%` }} />
                    </div>

                    <div className="bubp-checkout__await__countdown">
                        {timeToPayString[0]}&nbsp;
            ...
            {props.url && !props.shouldRedirectAutomatically && (
                <div className="bubp-checkout__await__app-link">
                    <ContentSeparator />
            ...
            {props.instructions && (
                <div className="bubp-checkout__await__instructions">
                    {typeof props.instructions === 'string' ? i18n.get(props.instructions) : props.instructions?.()}
                </div>
            )}

            {props.endSlot && <div className="bubp-checkout__await__end-slot">{props.endSlot()}</div>}
        </div>
    );
}
