...
describe('Installments', () => {
    ...
    test('does not render any installment options if the passed amount is 0', () => {
        ...
        expect(wrapper.find('.bubp-checkout__dropdown__element')).toHaveLength(0);
    });
    ...
    test('renders the right amount of installment options', () => {
        // card
        expect(getWrapper().find('.bubp-checkout__dropdown__element')).toHaveLength(2);

        expect(getWrapper({ brand: 'mc' }).find('.bubp-checkout__dropdown__element')).toHaveLength(3);
        expect(getWrapper({ brand: 'visa' }).find('.bubp-checkout__dropdown__element')).toHaveLength(4);
    });
    ...
    test('renders the radio button UI', () => {
        ...
        expect(getWrapper().find('.bubp-checkout__fieldset--revolving-plan')).toHaveLength(0);

        const wrapper = getWrapper({ brand: 'visa' });
        // containing div - once brand is set
        expect(wrapper.find('.bubp-checkout__fieldset--revolving-plan')).toHaveLength(1);
        // radio buttons
        expect(wrapper.find('.bubp-checkout__radio_group__input')).toHaveLength(3);
        // dropdown
        expect(wrapper.find('.bubp-checkout__dropdown__element')).toHaveLength(4);
    });

    test('does not render the radio button UI for mc', () => {
        const wrapper = getWrapper({ brand: 'mc' });
        // no containing div
        expect(wrapper.find('.bubp-checkout__fieldset--revolving-plan')).toHaveLength(0);
        // no radio buttons
        expect(wrapper.find('.bubp-checkout__radio_group__input')).toHaveLength(0);

        // dropdown
        expect(wrapper.find('.bubp-checkout__dropdown__element')).toHaveLength(3);
    });
});
